...
def test_storage_empty_folders_scan_json_output(tmp_path, monkeypatch) -> None:
    runner = CliRunner()
    ...
    result = runner.invoke(app, ["storage", "empty-folders", "scan", "--output", "json"])

    assert result.exit_code == 0
    payload = json.loads(result.stdout)
    assert payload["domain"] == "storage.empty-folders"
    assert payload["total_empty_dirs"] == 1
    assert payload["findings"][0]["relative_path"] == "empty-dir"
